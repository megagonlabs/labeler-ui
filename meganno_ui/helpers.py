...
from meganno_ui.constants import REQUEST_TIMEOUT_SECONDS
...
def get_request(path=None, json=None):
    if path is None or json is None:
        raise ValueError("get_request(): Both 'path' and 'json' cannot be None.")
    try:
        return requests_retry_session().get(
            path, json=json, timeout=REQUEST_TIMEOUT_SECONDS
        )
...
def post_request(path=None, json=None):
    if path is None or json is None:
        raise ValueError("post_request(): Both 'path' and 'json' cannot be None.")
    try:
